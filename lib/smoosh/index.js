...
var _write = {
  ...
  welcome: function (callback) {
    if (this.welcomed || _config.config.minimal) return callback && callback();
    this.welcomed = true;
    this.newLine();
    this.smoosh();
  },
  ...
  jshint: function(key, errors) {
    ...
    if (!errors.length) {
      if (!_config.config.minimal) {
        self.message('+ Congratulations. You are the perfect JavaScripter.'.green);
        self.message("  " + key.yellow + ' PASSED jshint with no errors!');
      }
    } else {
      self.message([
        '- Boo!',
      ...
      errors.forEach(function (err) {
        self.message('  ' + err.id + " line " + err.line + ": " + err.reason.yellow);
      });
      this.message('');
    }
  },
  ...
  analysis: function (type, file, oldLen, newLen){
    function minimalAnalysis() {
      var newLenStr = newLen.toString();
      if (newLen < oldLen) {
        newLenStr = newLenStr.green;
      } else if (newLen > oldLen) {
        newLenStr = newLenStr.red;
      }
      var isNow = "now " + newLenStr + " bytes";
      this.message(["+ " + type.magenta + " " + file.yellow + " was " + oldLen + " bytes,", isNow].join(" "));
    }

    function verboseAnalysis() {
    ...
    (_config.config.minimal ? minimalAnalysis : verboseAnalysis).call(this);
  },
